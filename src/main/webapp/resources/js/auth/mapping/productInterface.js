...
$(function () {
    ...
    function queryInfoParams(params) {
        return {
            interCode: $.trim($('#infoA').val()),
            interName: $.trim($('#infoA').val())
        };
    }
    ...
    function configInfoParams(params) {
        return {
            interCode: $.trim($('#infoB').val()),
            interName: $.trim($('#infoB').val())
        };
    }
    ...
    queryTable.bootstrapTable({
        url: Common.getRootPath() + '/admin/thirx/listNoPage.do',// 要请求数据的文件路径
        method: 'GET', // 请求方法
        cache: false,                       // 是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
        pagination: false,                   // 是否显示分页（*）
        sortable: true,                     // 是否启用排序
    ...
    $('#moveLeft').on('click', function (event) {
        ...
        $.ajax({
            type: "post",
            url: url,
            data: {"pdId": pdId,'interIds':interIds},
            dataType: 'json',
            cache : false,
            success: function (data, status) {
                if (data.status == Common.SUCCESS) {
    ...
    $('#remove').on('click',function () {
        ...
        $.each(tdata,function (index,value,array) {
            if(index == tLength - 1){
                ids += value.pdId +','+value.interId;
            }else{
                ids += value.pdId +','+value.interId +';';
            }
        });
        var pdId = tdata[0].pdId;
        var url = Common.getRootPath() + '/admin/pthirdinfo/removeMapping.do';
        $.ajax({
            type: "post",
            url: url,
            data: {'idList':ids},
