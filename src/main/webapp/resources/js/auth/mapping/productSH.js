...
$(function () {
    ...
    function queryInfoParams(params) {
        return {
            shName: $.trim($('#infoA').val()),
            shCode: $.trim($('#infoA').val())
        };
    }
    ...
    function configInfoParams(params) {
        return {
            shName: $.trim($('#infoB').val()),
            shCode: $.trim($('#infoB').val())
        };
    }
    ...
    queryTable.bootstrapTable({
        url: Common.getRootPath() + '/admin/hardware/listNoPage.do',// 要请求数据的文件路径
        method: 'GET', // 请求方法
        cache: false,                       // 是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
        pagination: false,                   // 是否显示分页（*）
        sortable: true,                     // 是否启用排序
    ...
    $('#moveLeft').on('click', function (event) {
        ...
        var shSelections = configTable.bootstrapTable('getSelections');
        if(!shSelections || shSelections.length <= 0){
            toastr.error('请先选择至少一条需要删除的软硬件信息');
            return ;
        }
        ...
        var shids = '';
        var cLength = shSelections.length;
        $.each(shSelections,function (index,value,array) {
            if(index == cLength - 1){
                shids += value.id;
            }else{
                shids += value.id +',';
    ...
    $('#moveRight').on('click', function (event) {
        ...
        if(!queryTableSelections || queryTableSelections.length <= 0){
            toastr.error('请先选择至少一条需要添加的软硬件信息');
            return ;
        }
