...
$(function () {
                ...
                floorName: {
                    ...
                    validators: {
                        ...
                        remote: {//ajax验证。server result:{"valid",true or false} 向服务发送当前input name值，获得一个json数据。例表示正确：{"valid",true}
                            url: Common.getRootPath() + '/admin/floors/existName.do',//验证地址
                            message: '楼层名称已存在',//提示消息
    ...
    function queryInfoByDataId(id) {
        var data = {};
        $.ajax({
            url: Common.getRootPath() + '/admin/floors/getById.do',
            data: {'id': id},
            type: "post",
            dataType: 'json',
            async: false,
    ...
    $('#infoTable').bootstrapTable({
        url: Common.getRootPath() + '/admin/floors/list.do',// 要请求数据的文件路径
        method: 'GET', // 请求方法
    ...
    $('#add').on('click', function () {
        ...
        $('#serialNo').val('');
        //清空验证信息
        $('#floorsForm').bootstrapValidator("destroy");
        validateForm();
    ...
    $('#infoTable').on('click', 'a[name="edit"]', function (e) {
        e.preventDefault();
        //清空验证信息
        $('#floorsModal').bootstrapValidator("destroy");
        validateForm();
        var vid = $(this).attr('aid');
        var data = queryInfoByDataId(vid);
        //赋值
        $('#floorsForm').initForm(data);
        $('#floorsModal').modal('show');
    });
    /**
     * 列表中按钮
    ...
    $('#infoTable').on('click', 'a[name="delete"]', function (e) {
        ...
        var data = queryInfoByDataId(vid);
        console.log(data);
        var videoName = data.floorName;
        var status = data.isDel;
        var alterName = '';
        if(status == '0'){
            alterName = '确认要启用楼层['+videoName+']吗？';
        }else{
            alterName = '确认要停用楼层['+videoName+']吗？';
        }
        Ewin.confirm({message: alterName }).on(function (e) {
            ...
            $.ajax({
                type: "post",
                url: Common.getRootPath() + '/admin/floors/modifyById.do',
    ...
    $('#save').on('click', function (e) {
        //阻止默认行为
        e.preventDefault();
        var bootstrapValidator = $("#floorsForm").data('bootstrapValidator');
        //修复记忆的组件不验证
        ...
        var url = '';
        if ($('#id').val().length == 0) {
            url = Common.getRootPath() + '/admin/floors/add.do';
        } else {
            url = Common.getRootPath() + '/admin/floors/update.do';
        }
        if (bootstrapValidator.isValid()) {
            $.ajax({
                url: url,
                data: $("#floorsForm").serialize(),
                type: "post",
                dataType: 'json',
                async: false,
                cache : false,
